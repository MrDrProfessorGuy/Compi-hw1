...
{letter}        BEGIN(STATE_ID);newString("");appendString(ID);
<STATE_ID>{alphanumeric}        appendString(ID);
<STATE_ID>{whitespace}          BEGIN(INITIAL);return(ID);
<STATE_ID>.                   printf("Error %c\n", yytext[0]);exit(0);
0                             newToken(NUM);
0[1-9]                        BEGIN(STATE_NUM);
<STATE_NUM>{digit}               appendString(NUM);
<STATE_NUM>{whitespace}          BEGIN(INITIAL);return(NUM);
<STATE_NUM>[.]                   printf("Error %c\n", yytext[0]);exit(0);
